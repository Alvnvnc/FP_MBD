    ...
    if connection:
        try:
            ...
            execute_sql_command(connection, function_total_event)
             ...
             # Function to calculate total schedule
            function_total_schedule = """
            CREATE FUNCTION CalculateTotalSchedules(team_id CHAR(6))
            RETURNS INT
            DETERMINISTIC
            BEGIN
                DECLARE total_schedules INT;
            ...
            """
            execute_sql_command(connection, function_total_schedule)

            # Function to calculate average player salary
            function_avg_player_salary = """
            CREATE FUNCTION CalculateTotalAverageSalary()
            RETURNS NUMERIC(10,2)
            DETERMINISTIC
            BEGIN
                DECLARE total_avg_salary NUMERIC(10,2);
                SELECT AVG(Jumlah_Pembayar) INTO total_avg_salary
                FROM Salary;
